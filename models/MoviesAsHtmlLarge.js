 ...
 * Constructor.
 * 
 * @class MoviesAsHTMLLarge
 * @constructor
 */
function MoviesAsHtmlLarge() {
  //
};
_.extend(MoviesAsHtmlLarge, BaseModel);

/**
 * MoviesAsHtmlLargefunction.
 ...
 */
MoviesAsHtmlLarge.prototype.getAsHtml = function(dir, includedependencies, responseHandler) {  
  var htmlReturn = "";
  BaseModel.getList(dir, function(movieList) {
		if(config.debug) util.log("Received from base: '" + movieList + "' at MoviesAsHtmlLarge.getAsHTML()");
        ...
        if(movieList.length == 0) {
            ...
            var handlebarsData = [{"Title": "None Found", "Year": "0000", "Poster": "http://" + config.serverHostname + ":" + config.serverPort + "/getImage/notValid.jpg", "noExists": "No movie found", "IncludeDependencies": includedependencies}];
            var source = fs.readFileSync('./views/MoviesAsHtmlLarge.template').toString();
		    ...
		    async.waterfall([
				  	    ...
				  	    function(err) {		  		
                            ...
                            if(jsonText == "[]") {
                                ...
                                waterfall_callback(null);
				  		        ...
				  		        try {
                                    var source = includedependencies ? fs.readFileSync('./views/MoviesAsHtmlLargeWithDependencies.template').toString() : fs.readFileSync('./views/MoviesAsHtmlLarge.template').toString();
						            var template = Handlebars.compile(source);

						            if(!handlebarsData[0].Response) handlebarsData = [{"Title": "None Found", "Year": "0000", "Poster": "http://" + config.serverHostname + ":" + config.serverPort + "/getImage/notValid.jpg", "noExists": "No movie found", "IncludeDependencies": includedependencies}];
						            for (var i = 0; i < handlebarsData.length; i++) {
							            if(handlebarsData[i].Poster != undefined) {
                                            ...
                                            handlebarsData[i].IncludeDependencies = includedependencies;
							            }
							        }
						            var wrapper = {objects: handlebarsData};
						            var htmlReturn = template(wrapper);	    
							        responseHandler(htmlReturn);
				  			        ...
				  			        if(config.debug) util.log('Failed to load ' + (includedependencies ? 'views/MoviesAsHtmlLargeWithDependencies.template: ' : 'views/MoviesAsHtmlLarge.template: ') + err);
				  			        responseHandler(err);				  			
				  		        }		    
				  		        waterfall_callback(null);	
                            }
			    ...
			    if(err && config.debug) util.log("Received error after async waterfall in MoviesAsHtmlLarge.getAsHTML(): " + err);
			    if(err) return err;
			    mongodb.close();
		    });
        }
...
module.exports = MoviesAsHtmlLarge;